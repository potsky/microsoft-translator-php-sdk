...
class GuardFile implements GuardInterface
{
    ...
    /**
     * This class stores Access Token in a file according to the client_key value
     *
    ...
    public function __construct($config = [], LoggerInterface $logger)
    {
        ...
        if (is_null($this->api_client_key)) {
            $this->logger->fatal(__CLASS__, 'init', 'api_client_key is mandatory');
        }
     ...
     * @return bool
     */
    public function hasAccessToken(array $param = [])
    {
     ...
     */
    public function storeAccessTokenForSeconds($access_token, $seconds, array $param = [])
    {
     ...
     * @return bool
     */
    public function deleteAccessToken(array $param = [])
    {
     ...
     */
    public function cleanAccessTokens(array $param = [])
    {
        $count = 0;
     ...
     * @return int the count of deleted Access Tokens
     */
    public function deleteAllAccessTokens(array $param = [])
    {
        $count = 0;
    ...
    {
        return $this->getDirectoryPath().DIRECTORY_SEPARATOR.self::FILE_PREFIX.sha1($this->api_base_url.'/'.$this->api_client_key);
    }
     ...
     * @return bool
     */
    private function save($access_token, $time)
    {
        unset($this->runtime_file_content);

        $bytes = @file_put_contents($this->getFilePath(), json_encode(['a' => $access_token, 'e' => $time]));
